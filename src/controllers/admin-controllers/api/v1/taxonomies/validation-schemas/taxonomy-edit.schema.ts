...
export const taxonomyEditSchema = [
  ...
  body( 'type' )
    .not()
    .isEmpty()
    .withMessage( validationMsgSerializer( "Taxonomy type is required", TaxonomyLocaleEnum.ERROR_SCHEMA_EDIT_TYPE ) ),
  body( 'term' )
    .not()
    .isEmpty()
    .withMessage( validationMsgSerializer( 'Taxonomy term is required', TaxonomyLocaleEnum.ERROR_SCHEMA_EDIT_TERM_EMPTY ) )
    .isLength( { max: 60 } )
    .withMessage( validationMsgSerializer(
      "Taxonomy term's length must be more than 1 and up to 60",
      TaxonomyLocaleEnum.ERROR_SCHEMA_EDIT_TERM_LENGTH
    ) ),
];
